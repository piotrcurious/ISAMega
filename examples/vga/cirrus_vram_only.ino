...
// Define off-screen VRAM test area
// Mode 13h uses 320*200 = 64000 bytes.
// CL-GD5422 typically has 256KB (0x40000 bytes) or 512KB.
// We'll start testing immediately after the visible screen buffer.
const uint32_t OFFSCREEN_VRAM_START = VRAM_START_ADDRESS + (uint32_t)SCREEN_WIDTH * SCREEN_HEIGHT;
const uint32_t VRAM_TEST_SIZE = 64 * 1024; // Test 64KB of off-screen VRAM
...
/**
 * @brief Performs a comprehensive VRAM read/write test on off-screen memory.
 * Reports overall PASS/FAIL and the first error address if found.
 */
void runVRAMTest() {
    Serial.println("\n--- Starting VRAM Test (Off-screen memory) ---");
    ...
    Serial.print("Testing 0x"); Serial.print(OFFSCREEN_VRAM_START, HEX);
    Serial.print(" to 0x"); Serial.print(OFFSCREEN_VRAM_START + VRAM_TEST_SIZE - 1, HEX);
    Serial.print(" ("); Serial.print(VRAM_TEST_SIZE / 1024); Serial.println("KB)");

    // --- Phase 1: Write pattern1 and verify ---
    Serial.println("Writing 0xAA and verifying...");
    for (uint32_t addr = OFFSCREEN_VRAM_START; addr < OFFSCREEN_VRAM_START + VRAM_TEST_SIZE; addr++) {
        memWrite(addr, pattern1);
    }
    ...
    for (uint32_t addr = OFFSCREEN_VRAM_START; addr < OFFSCREEN_VRAM_START + VRAM_TEST_SIZE; addr++) {
        ...
        if (readValue != pattern1) {
            testFailed = true;
            firstErrorAddr = addr;
            sprintf(buffer, "FAIL! Addr 0x%05lX (W:0x%02X R:0x%02X)\n", addr, pattern1, readValue);
            strcpy(vramTestResults, buffer); // Overwrite with error message
            Serial.print(buffer);
    ...
    if (!testFailed) {
        ...
        Serial.println("Phase 1: FAIL");
    }

    // --- Phase 2: Write pattern2 and verify ---
    if (!testFailed) { // Only proceed if Phase 1 passed
        Serial.println("Writing 0x55 and verifying...");
        for (uint32_t addr = OFFSCREEN_VRAM_START; addr < OFFSCREEN_VRAM_START + VRAM_TEST_SIZE; addr++) {
            memWrite(addr, pattern2);
        }
    ...
    if (testFailed) {
        // If an error occurred, the vramTestResults is already set.
        // Add a general failure message if it's not already there.
        if (strstr(vramTestResults, "FAIL") == NULL) { // Check if "FAIL" is not already present
...
/**
 * @brief Displays the stored VRAM test results on the VGA screen.
 */
void displayTestResultsOnVGA() {
    Serial.println("Displaying VRAM test results on VGA...");
    ...
    for (int y = 0; y < 50; y++) { // Clear top 50 rows
        for (int x = 0; x < SCREEN_WIDTH; x++) {
            setPixel(x, y, 0x00); // Clear with black
        }
    }

    drawString(10, startY, "VRAM Test Results:", 0xFF); // White text
    startY += lineHeight * 2;

    // Display VRAM Test Results
    // strtok modifies the string, so we need a copy if we want to reuse it.
    // For this simple case, it's fine as we only display it once.
    ...
    Serial.println("VRAM test results displayed on VGA.");
}
...
void setup()
{
    ...
    // 3. Perform the VRAM test on off-screen memory
    runVRAMTest();

    // 4. Display VRAM test results on the VGA screen
